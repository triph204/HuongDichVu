...
document.addEventListener("DOMContentLoaded", async function () {
  // ✅ Validate bàn trước khi load orders
...
async function validateAndLoadTable() {
  ...
  // Validate bàn trong localStorage
...
async function validateTableWithServer(tableNumber) {
  try {
    console.log("🔄 Orders: Đang validate bàn:", tableNumber);
    ...
    console.log("📡 Orders: Kết quả validate:", data);

    if (response.ok && data.valid) {
      localStorage.setItem("tableId", data.tableId);
      console.log("✅ Orders: Bàn hợp lệ:", data);
      return true;
    } else {
      console.warn("❌ Orders: Bàn không hợp lệ:", data.message);
      return false;
    }
  } catch (error) {
    console.error("❌ Orders: Lỗi khi validate bàn:", error);
